...
class InvitationMySqlDAO implements InvitationDAO{
	...
	public function insert($invitation){
		$sql = 'INSERT INTO invitation (user_id, household_id, pending, date_created) VALUES (?, ?, ?, ?)';
		$sqlQuery = new SqlQuery($sql);
	...
	public function update($invitation){
		$sql = 'UPDATE invitation SET user_id = ?, household_id = ?, pending = ?, date_created = ? WHERE inventation_id = ?';
		$sqlQuery = new SqlQuery($sql);
