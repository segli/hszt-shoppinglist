...
class BillMySqlDAO implements BillDAO{
	...
	public function insert($bill){
		$sql = 'INSERT INTO bill (date, cost, list_id, user_id) VALUES (?, ?, ?, ?)';
		$sqlQuery = new SqlQuery($sql);
		
		$sqlQuery->set($bill->date);
		$sqlQuery->set($bill->cost);
		$sqlQuery->set($bill->listId);
	...
	public function update($bill){
		$sql = 'UPDATE bill SET date = ?, cost = ?, list_id = ?, user_id = ? WHERE bill_id = ?';
		$sqlQuery = new SqlQuery($sql);
	...
	public function queryByDate($value){
		$sql = 'SELECT * FROM bill WHERE date = ?';
		$sqlQuery = new SqlQuery($sql);
		$sqlQuery->set($value);
		return $this->getList($sqlQuery);
	}
	...
	public function queryByListId($value){
		$sql = 'SELECT * FROM bill WHERE list_id = ?';
		$sqlQuery = new SqlQuery($sql);
		$sqlQuery->set($value);
		return $this->getList($sqlQuery);
	}
	...
	public function deleteByDate($value){
		$sql = 'DELETE FROM bill WHERE date = ?';
		$sqlQuery = new SqlQuery($sql);
		$sqlQuery->set($value);
		return $this->executeUpdate($sqlQuery);
	}
	...
	public function deleteByListId($value){
		$sql = 'DELETE FROM bill WHERE list_id = ?';
		$sqlQuery = new SqlQuery($sql);
		$sqlQuery->set($value);
		return $this->executeUpdate($sqlQuery);
	}
