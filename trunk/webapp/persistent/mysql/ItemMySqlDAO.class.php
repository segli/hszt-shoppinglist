...
class ItemMySqlDAO implements ItemDAO{
	...
	public function insert($item){
		$sql = 'INSERT INTO item (name, description, price, status, list_id) VALUES (?, ?, ?, ?, ?)';
		$sqlQuery = new SqlQuery($sql);
	...
	public function update($item){
		$sql = 'UPDATE item SET name = ?, description = ?, price = ?, status = ?, list_id = ? WHERE item_id = ?';
		$sqlQuery = new SqlQuery($sql);
	...
	public function deleteByListId($value){
		$sql = 'DELETE FROM item WHERE list_id = ?';
		$sqlQuery = new SqlQuery($sql);
		$sqlQuery->set($value);
		return $this->executeUpdate($sqlQuery);
	}
